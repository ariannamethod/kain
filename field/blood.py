...
if NICOLE2C_DIR.exists():
    sys.path.insert(0, str(NICOLE2C_DIR))
...
class BloodCCompiler:
    ...
    def compile_c_code(self, c_code: str, function_name: str = "main") -> Optional[str]:
        ...
        Для использования в трансформерах Field
        Использует бинарники из nicole_env или системные компиляторы
        """
        ...
        try:
            ...
            if not compiler_path:
                raise RuntimeError("No C compiler found. Install gcc, clang, or copy binaries to field/nicole_env/")
            ...
            # Компилируем с найденным компилятором
            compile_cmd = [
                compiler_path,
                '-O2',
                '-o', str(exe_file),
